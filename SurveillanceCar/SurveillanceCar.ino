  ...
  switch (type) 
  {
    ...
    case WS_EVT_DATA:
      ...
      if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) 
      {
        ...
        {
          // Core v3+: ledcWrite(pin, dutyCycle) - dùng pin thay vì channel number
          for (int i = 0; i < motorPins.size(); i++)
          {
            ledcWrite(motorPins[i].pinEn, valueInt);
          }
        }
        else if (key == "Light")
        {
...
void setUpPinModes()
{
  for (int i = 0; i < motorPins.size(); i++)
  {
    ...
    /* Attach PWM to pin EN of motor */
    ledcAttach(motorPins[i].pinEn, PWMFreq, PWMResolution);
  }
  moveCar(STOP);

  /* Attach PWM for LED flash */
